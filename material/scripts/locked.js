 ...
 var getFace = function(){
  ...
  return function ( faces, x, y ){
   ...
   while( i-- ){
    ...
    if( pointOfInteraction === undefined || intersection.z <= pointofinteraction="" ){="" face="faces[i];" }="" return="" face;="" }()="" domelement.addeventlistener(="" 'mousedown',="" oninteractstart="" );="" 'touchstart',="" api.update="function(){" direction.x="current.x" -="" start.x;="" direction.y="current.y" start.y;="" if(="" !axisdefined="" &&="" direction.length()=""> 30 ){
   axisDefined = true;
   sign = 1;
